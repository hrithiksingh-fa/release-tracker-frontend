        ...
        {(["workflows", "stages", "modules", "categories"] as Tab[]).map((t) => (
          <button
            ...
            className={`border-b-2 px-4 py-2 text-sm font-medium capitalize ${
              tab === t ? "border-[#5b8cff] text-white" : "border-transparent text-[#9aa1ac] hover:text-white"
            }`}
          >
            {t}
          </button>
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-end gap-2 rounded-xl border border-[#2a2f3a] bg-[#171a21] p-3">
        ...
        <Field label="Color">
          <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="h-9 w-14 rounded-lg border border-[#2a2f3a] bg-[#1e2229]" />
        </Field>
        ...
        {stages.map((s) => (
          <div key={s.id} className="flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-2">
              <span className="h-3 w-3 rounded-full" style={{ backgroundColor: s.color ?? "#9aa1ac" }} />
              <span className="text-sm font-medium text-white">{s.name}</span>
              {s.isDoneStage && <span className="rounded-full bg-[#33c17a]/15 px-2 py-0.5 text-[10px] font-semibold text-[#33c17a]">DONE</span>}
            </div>
            <button onClick={() => remove(s.id)} className="text-[#9aa1ac] hover:text-[#e05a5a]">
              <Trash2 size={14} />
            </button>
          </div>
        ))}
        {stages.length === 0 && <p className="px-4 py-6 text-center text-sm text-[#9aa1ac]">No stages yet.</p>}
      </div>
    </div>
  );
}
    ...
    <div className="flex gap-6">
      <div className="w-64 shrink-0">
        <button
          ...
          className="mb-3 flex w-full items-center justify-center gap-1.5 rounded-lg bg-[#5b8cff] px-3 py-2 text-sm font-semibold text-white hover:opacity-90"
        >
        ...
        <div className="flex flex-col gap-1">
          {(["CLIENT", "PHASE", "REQUIREMENT"] as WorkflowScope[]).map((scope) => (
            <div key={scope} className="mb-2">
              <div className="mb-1 px-1 text-[10px] font-semibold uppercase tracking-wide text-[#9aa1ac]">{scope}</div>
              {workflows
                ...
                .map((w) => (
                  <button
                    ...
                    className={`block w-full rounded-lg px-3 py-2 text-left text-sm ${
                      w.id === selectedId ? "bg-[#5b8cff]/15 text-white" : "text-[#9aa1ac] hover:bg-white/5"
                    }`}
                  >
                    {w.name}
                    {w.isTemplate && <span className="ml-1.5 text-[10px] text-[#e0a13a]">template</span>}
                  </button>
                ))}
            </div>
          ))}
      ...
      <div className="flex-1">
        {selected ? (
          ...
          <p className="text-sm text-[#9aa1ac]">Select a workflow, or create one.</p>
        )}
      </div>
...
function WorkflowDetail({
  ...
  return (
    <div className="rounded-xl border border-[#2a2f3a] bg-[#171a21] p-5">
      <div className="mb-1 flex items-center justify-between">
        <h2 className="text-base font-semibold text-white">{workflow.name}</h2>
        <button onClick={remove} className="text-[#9aa1ac] hover:text-[#e05a5a]" title="Delete workflow">
          <Trash2 size={16} />
        </button>
      </div>
      <p className="mb-4 text-xs text-[#9aa1ac]">
        {workflow.scope} scope{attachedClient && <> · attached to <span className="text-white">{attachedClient.name}</span></>}
      </p>

      <div className="mb-4 flex flex-wrap gap-2">
        {orderedStages.map((ws, i) => (
          <div key={ws.id} className="flex items-center gap-1 rounded-full border border-[#2a2f3a] bg-[#1e2229] py-1 pl-3 pr-1.5 text-xs">
            <span className="h-2 w-2 rounded-full" style={{ backgroundColor: ws.stage.color ?? "#9aa1ac" }} />
            <span className="text-white">{ws.stage.name}</span>
            <button onClick={() => move(i, -1)} disabled={i === 0} className="text-[#9aa1ac] hover:text-white disabled:opacity-30">
              <ArrowUp size={11} />
            </button>
            <button onClick={() => move(i, 1)} disabled={i === orderedStages.length - 1} className="text-[#9aa1ac] hover:text-white disabled:opacity-30">
              <ArrowDown size={11} />
            </button>
            <button onClick={() => removeStage(ws.id)} className="text-[#9aa1ac] hover:text-[#e05a5a]">
              <Trash2 size={11} />
            </button>
          </div>
        ))}
        {orderedStages.length === 0 && <p className="text-xs text-[#9aa1ac]">No stages in this workflow yet.</p>}
      </div>
      ...
      <div className="mb-5 flex gap-2">
        ...
        <button onClick={addStage} disabled={!addStageId} className="rounded-lg border border-[#2a2f3a] px-3 py-2 text-sm text-[#9aa1ac] hover:border-[#5b8cff] hover:text-white disabled:opacity-50">
          Add
        </button>
      </div>
      ...
      {workflow.scope !== "CLIENT" && (
        <div>
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-[#9aa1ac]">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={attach} disabled={!attachClientId} className="rounded-lg border border-[#2a2f3a] px-3 py-2 text-sm text-[#9aa1ac] hover:border-[#5b8cff] hover:text-white disabled:opacity-50">
              Attach
            </button>
          </div>
        </div>
...
function CreateWorkflowModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm rounded-xl border border-[#2a2f3a] bg-[#171a21] p-6">
        <h2 className="mb-4 text-base font-semibold text-white">New workflow</h2>
...
function ModulesTab() {
  ...
  return (
    <div>
      <p className="mb-4 text-xs text-[#9aa1ac]">Shown as a multi-select on the client form ("modules they use").</p>
      <div className="mb-4 flex flex-wrap items-end gap-2 rounded-xl border border-[#2a2f3a] bg-[#171a21] p-3">
        <Field label="Name">
        ...
        {modules.map((m) => (
          <div key={m.id} className="flex items-center justify-between px-4 py-3">
            <div>
              ...
              {m.description && <div className="text-xs text-[#9aa1ac]">{m.description}</div>}
            </div>
            <button onClick={() => remove(m.id)} className="text-[#9aa1ac] hover:text-[#e05a5a]">
              <Trash2 size={14} />
            </button>
          </div>
        ))}
